...
class TransArchiveTaskFix extends FannieTask
{
    public $name = 'Transaction Archiving Fix';

    public $description = 'Recover from errors in
        Archive current transaction data.
...
echo $this->cronMsg("To do: {$dates[0]}");
//return true;
...
$summary_source = 'core_trans.dlog_15';
$delnum = 0;
$insertnum = 0;
//            $summary_source = DTransactionsModel::selectDlog($date);
            ...
            if ($sql->table_exists("sumUpcSalesByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumUpcSalesByDay WHERE tdate='$date'");
                if ($rslt === False) {
                    echo $this->cronMsg("D$delnum failed for date: {$date}.");
                    return true;
                }
                $rslt = $sql->query("INSERT INTO sumUpcSalesByDay
                    ...
                    GROUP BY upc");
                if ($rslt === False) {
                    echo $this->cronMsg("I$insertnum failed for date: {$date}.");
                    return true;
                } else {
                    echo $this->cronMsg("I$insertnum OK.");
                }
            }
            ...
            if ($sql->table_exists("sumRingSalesByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumRingSalesByDay WHERE tdate='$date'");
                $rslt = $sql->query("INSERT INTO sumRingSalesByDay
                ...
                if ($rslt === False) {
                    echo $this->cronMsg("I$insertnum failed.");
                    return true;
                } else {
                    echo $this->cronMsg("I$insertnum OK.");
                }
            ...
            if ($sql->table_exists("sumDeptSalesByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumDeptSalesByDay WHERE tdate='$date'");
                $rslt = $sql->query("INSERT INTO sumDeptSalesByDay
            ...
            if ($sql->table_exists("sumMemSalesByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumMemSalesByDay WHERE tdate='$date'");
                $rslt = $sql->query("INSERT INTO sumMemSalesByDay
            ...
            if ($sql->table_exists("sumMemTypeSalesByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumMemTypeSalesByDay WHERE tdate='$date'");
                $rslt = $sql->query("INSERT INTO sumMemTypeSalesByDay
            ...
            if ($sql->table_exists("sumTendersByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumTendersByDay WHERE tdate='$date'");
                $rslt = $sql->query("INSERT INTO sumTendersByDay
            ...
            if ($sql->table_exists("sumDiscountsByDay") && $sql->dbms_name() != 'mssql') {
                $delnum++; $insertnum++;
                $rslt = $sql->query("DELETE FROM sumDiscountsByDay WHERE tdate='$date'");
                $rslt = $sql->query("INSERT INTO sumDiscountsByDay
                    SELECT DATE(MAX(tdate)) AS tdate, c.memType,
                    CONVERT(SUM(total),DECIMAL(10,2)) as total,
                    COUNT(DISTINCT trans_num) AS transCount
                    FROM $summary_source AS t
