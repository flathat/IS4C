...
class EquityRefund extends \COREPOS\Fannie\API\member\MemberModule
{

    protected $moduleHeading = 'Equity Refund';
...
.MemFormTable td {
	padding: 2px 2px 2px 2px;
        ...
        $ret .= "<tr>";
		$ret .= "<th title='Allow/prevent the member to Equity Refund.'>Equity Refund OK</th>";
	...
	function SaveFormData($memNum){
        ...
        // Enforce the Equity Refund limit.
        ...
        if ("{$charge_ok}|{$limit}" != FormLib::get_form_value('pp_orig_values','')) {
            if ($charge_ok && $limit > 0) {
                return '<p class="memMessage">**Problem in Equity Refund: Equity Refund accounts may not have Limit of more than 0.' .
            ...
            if ($test === False) {
                return 'Error: Problem saving Equity Refund limit<br />';
            } else {
                return '';
            }
        } else {
