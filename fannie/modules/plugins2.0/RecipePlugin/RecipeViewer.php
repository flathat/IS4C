...
class RecipeViewer extends FannieRESTfulPage
{
    ...
    private function extractIngredients($lines)
    {
        ...
        foreach ($lines as $line) {
             ...
             * if (!empty(trim($line)) && !$found) { echo "A"; }
             * PHP Fatal error:  Can't use function return value in write context in plugins2.0/RecipePlugin/RecipeViewer.php on line 146
             * Won't lint-check in PHP 5.4.39 but is OK in PHP 5.5.9
             */
            $trim_line = trim($line);
            if (!empty($trim_line) && !$found) {
