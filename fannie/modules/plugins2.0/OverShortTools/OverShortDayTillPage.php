...
class OverShortDayTillPage extends FanniePage {
	...
	function preprocess(){
		...
		// Use the site default if set.
		if (isset($FANNIE_WINDOW_DRESSING)) {
			$this->has_menus($FANNIE_WINDOW_DRESSING);
		}
		$this->title = "Over/Short each Till";
		$this->header = "Over/Short each Till";
	...
	function ajaxRequest($action){
		...
		case 'date':
			...
			if (FormLib::get_form_value('emp_no') !== ''){
				...
				/* EL Determine which registers were used.
				      Assumes as pseudo-employees.emp_no can join on dtrans.register_no
				$empsQ = "SELECT e.firstname, d.emp_no " .
									"FROM $dlog as d, $FANNIE_OP_DB" . $dbc->sep() ."employees as e " .
				...
				*/
				$empsQ = "SELECT e.firstname, e.emp_no " .
									"FROM $dlog as d, $FANNIE_OP_DB" . $dbc->sep() ."employees as e " .
									"WHERE d.tdate BETWEEN ? AND ? " .
										"AND trans_type='T' " .
			...
			$output .= "<th>Till</th><th>&nbsp;</th><th>Total</th><th>Counted Amt</th><th>Over/Short</th></tr>";

			/* EL Get the tender types that were used.
			*/
			$tQ = "SELECT d.trans_subtype, t.TenderName ".
						"FROM $dlog as d, "
			...
			EL: select on d.register_no. Was:
			$q = "SELECT -1*sum(total) AS total,
				emp_no,
				CASE WHEN trans_subtype IN ('CC','AX') THEN 'CC' ELSE trans_subtype END
					AS trans_subtype
				FROM $dlog
				WHERE tdate BETWEEN ? AND ? AND trans_type='T' ";
			// If for a single employee, then restrict to him/her.
			if (FormLib::get_form_value('emp_no') !== ''){
				$q .= ' AND emp_no=? ';
			...
			*/
			...
			while ($row = $dbc->fetch_array($empsR)){
				...
				$output .= "<tr><td><a href=OverShortDayTillPage.php?action=date&arg=$date&emp_no=$row[1] target={$date}_{$row[1]}>$row[0]</a></td>";
...
a {
  color: blue;
	...
	function body_content(){
		...
		<p>
		<b>Date to reconcile</b>:<input type=text id=date onfocus="showCalendarControl(this);" />
