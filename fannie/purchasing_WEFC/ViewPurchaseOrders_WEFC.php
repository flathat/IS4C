 ...
 */
/*
 * This requires (if used) $FANNIE_PO_EMAIL and $FANNIE_PO_EMAIL_NAME,
 *  and refers to but does not require $FANNIE_PO_PREFIX,
 *  and $FANNIE_DEFAULT_PO_EXPORT.
 * There is not Install-level UI for these yet, they have to be
 *  hand entered in config.php.
 */
...
class ViewPurchaseOrders_WEFC extends FannieRESTfulPage 
{
    protected $header = 'Purchase Orders WEFC';
    protected $title = 'Purchase Orders WEFC';

    public $description = '[View Purchase Orders WEFC] lists pending orders and completed invoices.';

    protected $must_authenticate = true;

    private $show_all = true;
    private $REST_log = false;
    private $self_page = '';
    private $home_page = '';
    private $page_suffix = '';
    ...
    public function preprocess()
    {
        $this->addRoute(
            ...
            'post<id><sku><qty><cost><receiveAll>',
            'post<id><note>',
            'post<id><sku><isSO>',
            'post<id><sku><adjust>',
            'get<merge>'
    ...
    */
    protected function get_merge_handler()
    {
        ...
        return $this->self_page . '?init=pending';
        //was: return 'ViewPurchaseOrders.php?init=pending';
    }
    ...
    */
    ...
    protected function get_id_sendAs_handler()
    {
        ...
        return $this->self_page . '?id=' . $this->id;
        //was: return 'ViewPurchaseOrders.php?id=' . $this->id;
    }
    ...
    protected function get_orders($placed, $store=0, $month=0, $year=0)
    {
        ...
        $query = 'SELECT p.orderID, p.vendorID, MIN(creationDate) as creationDate,
                ...
                SUM(i.receivedTotalCost) as receivedCost,
                v.vendorName,
                MAX(i.receivedDate) as receivedDate,
                MAX(p.vendorOrderID) AS vendorOrderID,
                MAX(p.vendorInvoiceID) AS vendorInvoiceID,
                MAX(s.description) AS storeName,
                MAX(p.placed) AS placed,
                SUM(CASE WHEN isSpecialOrder != 0 THEN i.quantity ELSE 0 END) AS soFlag
        ...
        while ($row = $dbc->fetchRow($result)) {
            /* EL Why is vendorName "0" */
            if ($row['vendorName'] == 0) {
                $pvn = $dbc->prepare("SELECT vendorName FROM vendors WHERE vendorID = ?");
                $pvr = $dbc->execute($pvn,array($row['vendorID']));
                $pvrr = $dbc->fetchRow($pvr);
                $row['vendorName'] = $pvrr['vendorName'];
            }
            if ($row['placed']) {
        ...
        foreach ($mergable as $m) {
            ...
            $tPending .= sprintf('<a href="%s?%s">Merge %s Orders</a><br />',
                $this->self_page,
                $idStr, $m['name']);
            //was: $tPending .= sprintf('<a href="ViewPurchaseOrders.php?%s">Merge %s Orders</a><br />', $idStr, $m['name']);
        }
        $tPending .= '</div>';
    ...
    /*
     * EL Change to soFlag handling may be a bugfix
     * EL Add PO# = vendorOrderID, with link
     * EL Add Invoice# = vendorInvoiceID, with link
     * EL Change to $placed handling may be a bugfix
                ($placed == 1 ? $row['placedDate'] : '&nbsp;'), // doesn't work
     * EL Use:
     date('Y-m-d', strtotime($row['creationDate'])), instead of entire date
                $row['orderID'], $row['creationDate'],
        Does time matter?
     */
    private function orderRowToTable($row, $placed)
    {
        return sprintf('<tr %s>
                <td><a href="%s?id=%d">%s</a></td>
                <td><a href="%s?id=%d">%s</a></td>
                <td><a href="%s?id=%d">%s</a></td>
                <td>%s</td>
                <td>%s</td>
                ...
                ($row['soFlag'] > 0.00 ? 'class="success" title="Contains special order(s) ' . $row['soFlag'] . '"' : 'class="success"'),
                $this->self_page,
                ...
                $row['records'],
                $row['estimatedCost'],
                ((!empty($placed) && $row['placedDate'] != '') ? date('Y-m-d', strtotime($row['placedDate'])) : '&nbsp;'),
        ...
        return sprintf('<tr %s>
                ...
                ($row['soFlag'] > 0.00 ? 'class="success" title="Contains special order(s) ' . $row['soFlag'] . '"' :
                'class="success"'),
                $row['orderID'], date('Y-m-d', strtotime($row['creationDate'])),
                $row['orderID'], $row['vendorOrderID'],
                $row['orderID'], $row['vendorInvoiceID'],
     ...
     * EL: New, with cost
     */
    protected function post_id_sku_qty_cost_receiveAll_handler()
    {
        $dbc = $this->connection;
    ...
    /* Original with calculated receivedTotalCost
     */
    ...
    protected function post_id_sku_recode_handler()
    {
        ...
        return $this->self_page . '?id=' . $this->id;
        //was; return filter_input(INPUT_SERVER, 'PHP_SELF') . '?id=' . $this->id;
    }
    ...
    protected function get_id_recode_view()
    {
        ...
        $ret .= '</table>
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="' . $this->self_page .
            '?id=' . $this->id . '" class="btn btn-default">Back to Order</a>
            </p>
        </form>';
            /* was:
        $ret .= '</table>
    ...
    /*
     * EL: REST_log
     * EL: Column Head 'Ord. Qty' instead of 'Cases', to be like 'Recd. Qty'
     */
    protected function get_id_view()
    {
        ...
        $dbc->selectDB($this->config->get('OP_DB'));
        if ($this->REST_log) {
            $dbc->logger("REST: get_id_view() id: $this->id");
        }
        ...
        $exportOpts = '';
        //was:foreach (COREPOS\Fannie\API\item\InventoryLib::orderExporters() as $class => $name) { }
        foreach ($this->orderExporters() as $class => $name) {
            $selected = ($class === $this->config->get('DEFAULT_PO_EXPORT') . $this->page_suffix)
                ? 'selected' : '';
        ...
        /* EL: Less geeky date format
        <b>Created</b>: {$orderObj->creationDate}
        <b>Placed</b>: <span id="orderPlacedSpan">{$orderObj->placedDate}</span>
         */
        $c_date = str_replace(' 00:00:00','', $orderObj->creationDate);
        $p_date = str_replace(' 00:00:00','', $orderObj->placedDate);
...
<p>
    <div class="form-inline">
        ...
        <b>Created</b>:&nbsp;{$c_date}
        &nbsp;&nbsp;&nbsp;&nbsp;
        <b>Placed</b>:&nbsp;<span id="orderPlacedSpan">{$p_date}</span>
        ...
        Export&nbsp;as:&nbsp;<select id="exporterSelect" class="form-control input-sm">
            ...
            onclick="doExport({$this->id});return false;">Export</button>
        {$exportEmail}
        &nbsp;&nbsp;&nbsp;
        <a type="button" class="btn btn-default btn-sm" 
            href="{$this->self_page}?{$init}">All Orders</a>
        &nbsp;&nbsp;&nbsp;
        <a type="button" class="btn btn-default btn-sm" 
            href="{$this->home_page}">Home</a>
    </div>
</p>
<div class="row">
    <div class="col-sm-6">
    ...
    <p>
        <a class="btn btn-default btn-sm {$pendingOnlyClass}"
            href="EditOnePurchaseOrder{$this->page_suffix}.php?id={$this->id}">Add Items</a>
        ...
        <a class="btn btn-default btn-sm {$placedOnlyClass}"
            href="ManualPurchaseOrderPage{$this->page_suffix}.php?id={$orderObj->vendorID}&adjust={$this->id}">Edit Order</a>
        <span class="{$placedOnlyClass}">&nbsp;&nbsp;&nbsp;&nbsp;</span>
        <a class="btn btn-default btn-sm {$placedOnlyClass}" id="receiveBtn"
            href="{$this->self_page}?id={$this->id}&receive=1">Receive Order</a>
        <span class="{$placedOnlyClass}">&nbsp;&nbsp;&nbsp;&nbsp;</span>
        <a class="btn btn-default btn-sm {$placedOnlyClass}" id="receiveBtn"
            href="TransferPurchaseOrder{$this->page_suffix}.php?id={$this->id}">Transfer Order</a>
        <span class="{$placedOnlyClass}">&nbsp;&nbsp;&nbsp;&nbsp;</span>
        <a class="btn btn-default btn-sm {$placedOnlyClass}"
            href="{$this->self_page}?id={$this->id}&recode=1">Alter Codings</a>
    </p>
...
HTML;
        ...
        foreach ($model->find() as $obj) {
            ...
            $totalOrderedCost += ($obj->quantity() * $obj->caseSize() * $obj->unitCost());
            $totalReceivedCost += $obj->receivedTotalCost();
        ...
        /* EL: Ordered and Received Totals
         */
        $ret .= sprintf('<tr><td><strong>Totals</strong></td>
            <td colspan=7> &nbsp; </td>
        ...
        $this->add_script('js/view' . $this->page_suffix . '.js');
        //was:$this->add_script('js/view.js');
    ...
    */
    protected function get_id_receive_view()
    {
        ...
        $ret = '
            <p>Receiving <a href="' .$this->self_page . '?id=' . $this->id .
            '">Order # ' . $po_prefix . $this->id . '</a></p>
            <p><div class="form-inline">
                ...
                <a href="?id=' . $this->id .
                '&receiveAll=1" class="btn btn-default btn-reset">All</a>
                &nbsp;&nbsp;&nbsp;
                <a type="button" class="btn btn-default btn-sm" 
                    href="' . $this->home_page . '">Home</a>
                </form>
    ...
    protected function get_id_receiveAll_view()
    {
        ...
        $po_prefix = $this->config->get('PO_PREFIX');
        $ret = '';
        $ret .= '<p>Receiving <a href="' . $this->self_page . '?id=' .$this->id .
            '">Order # ' . $po_prefix . $this->id . '</a></p>';
        $ret .= '<form method="post">
            ...
            <tr>
                ...
                <th>Qty Ordered</th>
                <th>Qty Received</th>
                <th>Total Cost Rec\'d</th>
            </tr>';
        foreach ($poi->find() as $item) {
            $qty = $item->caseSize() * $item->quantity();
            $cost = $item->caseSize() * $item->quantity() * $item->unitCost();
            $ret .= sprintf('<tr>
                ...
                <td><input type="text" class="form-control input-sm" name="cost[]" value="%.2f" /></td>
                </tr>',
                $item->sku(), $item->sku(),
                $item->brand(),
                $item->description(),
                ...
                ($item->receivedTotalCost() === null ? $cost : $item->receivedTotalCost())
            );
        }
        $ret .= '</table>
            <p>
    ...
    /*
     * EL: Show and link PO id OLD, no cost
    protected function get_id_receiveAll_view()
    {
        $dbc = FannieDB::getReadOnly($this->config->get('OP_DB'));
        $poi = new PurchaseOrderItemsModel($dbc);
        ...
        $po_prefix = $this->config->get('PO_PREFIX');
        $ret = '';
        $ret .= '<p>Receiving <a href="ViewPurchaseOrders.php?id=' .$this->id .
            '">Order # ' . $po_prefix . $this->id . '</a></p>';
        $ret .= '<form method="post">
            <input type="hidden" name="id" value="' . $this->id . '" />
            <input type="hidden" name="receiveAll" value="1" />
    ...
    */
    ...
    protected function get_view()
    {
        ...
        if ($this->REST_log) {
            $this->connection->logger("REST: get_view() init: $init");
        }
        ...
        $this->addScript($this->config->get('URL') . 'src/javascript/jquery.floatThead.min.js');
        $this->addScript('js/view' . $this->page_suffix . '.js');
        //was:$this->addScript('js/view.js');
        $this->addOnloadCommand("\$('.tablesorter').tablesorter();\n");
        $this->addOnloadCommand("\$('.table-float').floatThead();\n");
...
<div class="form-group form-inline">
    ...
    &nbsp;
    <button class="btn btn-default" onclick="location='{$this->home_page}';
...
HTML;
    ...
    protected function orderExporters()
    {
        $ret = array();
        $path = dirname(__FILE__) . '/exporters/';
        //was:$path = dirname(__FILE__) . '/../../purchasing/exporters/';
        $dir = opendir($path);
    ...
    /* EL: WEFC formatting and a tiny bit of content.
     * WHY?: Edit Order only for Placed orders?
     * WHY?: Codings Totals only for Received Items?
     */
    public function helpContent()
    {
        ...
        if ($this->config->get('COOP_ID') == 'WEFC_Toronto') {
            ...
            if (isset($this->receive)) {
                $ret .= '<p>Receive an order.
                    <ul>
                    <li>First enter a SKU (or UPC) to see the quantities that were ordered.
                    </li>
                    <li>Then enter the actual quantities received as well as costs.
                    </li>
                    <li>If a received item was <strong>not</strong> on the original order,
                    you will be prompted to provide additional information
                    so the item can be added to the order.
                    </li>
                    </ul>
                </p>';
            } elseif (isset($this->id)) {
                $ret .= '<p>Details of a Purchase Order.
                    <ul>
                    <li>Tick "Placed" to change the status of the Order from
                    Pending to Placed.
                    </li>
                    <li>"Coding(s)" come from POS department <em>Sales Codes</em>,
                        which connect departments to the organization\'s chart of
                        accounts.
                        They are optional and are entered in Manage Departments.
                        <br />The Codings Totals are only calculated for items
                        in orders that have been Placed.
                    </li>
                    <li>"Export" outputs the order data in various formats.
                    </li>
                    <li> "Edit Order" is only available for Placed Orders.
                    It loads the order line-items into an editing
                    interface where adjustments to all fields can be made.
                    </li>
                    <li>"Receive Order" is used to resolve a purchase order with
                    actual quantities received.
                    </li>
                    <li>Pink rows: Once an Order has been marked as "Placed"
                    the next time it is displayed the rows for items that have
                    not been received yet are pink.
                    </li>
                    </ul>
                </p>';
            } else {
                $ret .= '<p>The lists of Orders.
                    <ul>
                    <li>There are tabs for "Pending" (In Progress) and "Placed" Orders.
                    </li>
                    <li>Use the dropdowns to filter the list.
                        <ul>
                        <li>In "Showing" the distinction  between
                        <em>All Orders</em> and <em>My Orders</em> only works if user
                        authentication is enabled.
                        </li>
                        <li>In "During" the the date used is the Creation Date,
                        not the date the order was placed.
                        </li>
                        </ul>
                    </li>
                    <li>Click the Created Date or PO# or Invoice# link to view
            ...
            if (isset($this->receive)) {
                $ret .= '<p>Receive an order. First enter a SKU (or UPC) to see
                ...
                $ret .= '<p>Details of a Purchase Order. Coding(s) are driven by POS department
                ...
                $ret .= '<p>Click the date link to view a particular purchase order. Use
                the dropdowns to filter the list. The distinction between <em>All Orders</em>
                and <em>My Orders</em> only works if user authentication is enabled.
                </p>';
            }
        }
