...
class TransferPurchaseOrder_WEFC extends FannieRESTfulPage 
{
    protected $header = 'Transfer Order WEFC';
    protected $title = 'Transfer Order WEFC';

    public $description = '[Transfer Purchase Orders WEFC] moves some or all items from one order to another store.';
    ...
    protected function post_handler()
    {
        ...
        if (count($skus) === 0 || empty($from) || empty($dest) || $from === $dest) { 
            ...
            //was:return 'TransferPurchaseOrder.php';
        }
        ...
        return 'ViewPurchaseOrders' . $this->page_suffix . '.php?id=' . $destID;
        //was:return 'ViewPurchaseOrders.php?id=' . $destID;
    }
    ...
    protected function post_id_handler()
    {
        ...
        if (count($skus) === 0 || empty($from) || empty($dest) || $from === $dest) { 
            // nothing selected, bad IDs, same IDs
            return $this->self_page;
            //was:return 'TransferPurchaseOrder.php?id=' . $this->id;
        }
    ...
    protected function get_id_view()
    {
        $dbc = $this->connection;
        $dbc->selectDB($this->config->get(('OP_DB')));
        $this->addScript('js/transfer' . $this->page_suffix . '.js');
        //was:$this->addScript('js/transfer.js');
